...
// http://javascript-source.com
//***********************************************
jQuery.validator.addMethod("pattern",function(c,b,a){return this.optional(b)||(new RegExp("^(?:"+a+")$")).test(c)},jQuery.format("Please enter the correct value"));jQuery.validator.origRequired=jQuery.validator.methods.required;jQuery.validator.addMethod("required",function(c,b,a){if(/^url$/.test(b.getAttribute("type"))&&($.trim(c)=="http://")){return false}return jQuery.validator.origRequired.call(this,c,b,a)});jQuery.validator.addMethod("name",function(c,b,a){if(/^name\[first\]|name\[last\]$/.test(b.getAttribute("name"))){return this.optional(b)||(new RegExp("^[^0-9\\<\\>\\\\/\\|*\\=\\_\\+\\~\\!\\?\\@\\#\\№\\$\\%\\^\\&\\(\\)\\{\\}\\[\\]\\;\\:\\`\\,]+$")).test(c)}return true},jQuery.format("Please enter the correct characters"));jQuery(function(){var a=jQuery;form=a(".formoid-solid-blue");form.validate({errorClass:"error-label",wrapper:"label",errorElement:"span",ignore:"",success:function(b,c){if(b.parent().hasClass("tooltip")){b.parent().remove()}else{b.remove()}var d=a(c).parent();d.removeClass("error-field")},errorPlacement:function(b,c){var d=a(c).parent();d.addClass("error-field");b.css("marginLeft",0).appendTo(d||c).addClass("tooltip").addClass("bottom").addClass("error").append(a("<span>").addClass("tooltip-arrow"))}});jQuery("[required=required]").keyup(function(){a(this).valid()})});